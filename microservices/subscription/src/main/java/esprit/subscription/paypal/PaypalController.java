...
import esprit.subscription.dao.CancellationReasonDao;
...
public class PaypalController {
    ...
    private final CancellationReasonDao cancellationReasonDao; // Add this
    ...
    public ResponseEntity<String> cancelSubscription(
        ...
        try {
            log.info("Attempting to cancel subscription with subid: {}", subid);
            Subs subscription = subsRepository.findById(subid)
                    .orElseThrow(() -> new IllegalArgumentException("Subscription not found"));
            ...
            log.info("Updating subscription status to canceled for subid: {}", subid);
            subsRepository.save(subscription);

            // Save cancellation reasons
            for (String reason : reasons) {
                CancellationReason cancellationReason = new CancellationReason();
                cancellationReason.setSubid(subid);
                cancellationReason.setReason(reason);
                cancellationReason.setTimestamp(LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
                cancellationReasonDao.save(cancellationReason);
                log.info("Saved cancellation reason for subid {}: {}", subid, reason);
            }
    ...
    // New endpoint to get cancellation reasons statistics
    @GetMapping("/subs/cancellation-reasons")
    public ResponseEntity<List<CancellationReason>> getCancellationReasons() {
        try {
            List<CancellationReason> reasons = cancellationReasonDao.findAll();
            return ResponseEntity.ok(reasons);
        } catch (Exception e) {
            log.error("Error fetching cancellation reasons: {}", e.getMessage(), e);
